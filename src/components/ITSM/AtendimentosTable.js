...
const AtendimentosTable = ({ titulo, apiUrl, filtrosExtras = {}, tipoTela, filtro }) => {
        ...
        const fetchFilterOptions = async (hubs = [], categoria = [], subcategoria = []) => {
            try {
                const responses = await Promise.all([
                    ...
                    axios.get(`${process.env.REACT_APP_API_BASE_URL}/tickets/form/status-tickets`, { headers: { 'Authorization': `Bearer ${token}`, 'X-User-Email': user.email } }),
                    ...
                    hubs.length > 0 ? axios.get(`${process.env.REACT_APP_API_BASE_URL}/tickets/form/unidade?hub=${hubs.join(',')}`, { headers: { 'Authorization': `Bearer ${token}`, 'X-User-Email': user.email } }) : Promise.resolve({ data: [] }),
                    categoria.length > 0 ? axios.get(`${process.env.REACT_APP_API_BASE_URL}/tickets/form/subcategorias?categoria=${categoria[0]}`, { headers: { 'Authorization': `Bearer ${token}`, 'X-User-Email': user.email } }) : Promise.resolve({ data: [] }),
                    (categoria.length > 0 && subcategoria.length > 0) ? axios.get(`${process.env.REACT_APP_API_BASE_URL}/tickets/form/assuntos?categoria=${categoria[0]}&subcategoria=${subcategoria[0]}`, { headers: { 'Authorization': `Bearer ${token}`, 'X-User-Email': user.email } }) : Promise.resolve({ data: [] })
                ]);
    ...
    useEffect(() => {
        const fetchSavedFilters = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/tickets/${filtro}/${user.id_user}`, { headers: { 'Authorization': `Bearer ${token}`, 'X-User-Email': user.email } });
    ...
    useEffect(() => {
        ...
        if (cod_fluxo) {
            const fetchTicketData = async () => {
                try {
                    const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/tickets/ticket?cod_fluxo=${cod_fluxo}`, { headers: { 'Authorization': `Bearer ${token}`, 'X-User-Email': user.email } });
    ...
    const handleClick = async (ticket) => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/tickets/ticket?cod_fluxo=${ticket.cod_fluxo}`, { headers: { 'Authorization': `Bearer ${token}`, 'X-User-Email': user.email } });
            setModalData(response.data);
            setShowModal(true);
        } catch (error) {
